	...
	for x in range(width):
		for y in range(width):
			for z in range(width):
				print('O' if arr[x][y][z]<=0.5 else '@',end=' ')
			print("")
		print("")


def print3DModel(arr):
	from main import width
	...
	for x in range(0,width, 3):
		for y in range(0,width, 3):
			for z in range(0,width, 3):
				m = 'o'
				if arr[x][y][z]>=1:
					ax.scatter(x, y, z, marker=m)
					tot+=1
	# plt.savefig('op.pdf', bbox_inches='tight')
	plt.show()

def makeImage(arr,slice):
	from PIL import Image
	import numpy as np
	width = len(arr)

	pixels = [[0 for x in range(width)] for y in range(width)]

	for x in range(width):
		for y in range(width):
			# pixels[x][y] = (255,255,255) if arr[x][y]>=1 else (0,0,0)
			pixels[x][y] = (arr[x][y]*255,arr[x][y]*255,arr[x][y]*255)
	...
	new_image.save("gif/"+str(slice)+'.png')


def makegif(arr,width):
...
def getMidPoint(origin,index):
	ans = [0 for i in range(3)]
	for i in range(3):
		ans[i] = origin[i]+addToOriginForMid[index][i]
	return ans
...
def getVert(origin,index):
	ans = [0 for i in range(3)]
	for i in range(3):
		ans[i] = origin[i]+addToVertice[index][i]
	return ans
...
def getIndex(arr, index):
	val = 0
	for i in range(8):
		temp = getVert(index,i)
		val+= 2**i if arr[temp[0]][temp[1]][temp[2]]>=1 else 0
	return val
...
def makeFacets(arr):
	...
	for x in range(width):
		for y in range(width):
			for z in range(width):
				ind = getIndex(arr,[x,y,z])
				for el in cases[ind]:
					temparr = [[0 for i in range(3)] for j in range(3)]
					for i in range(3):
						temparr[i] = getMidPoint([x,y,z],el[i])
